...
extension SavedItem {
    ...
    func update(from edge: SearchSavedItemsQuery.Data.User.SearchSavedItems.Edge, with space: Space) {
        cursor = edge.cursor

        let savedItemParts = edge.node.savedItem.fragments.searchSavedItemParts

        update(from: savedItemParts, with: space)
    }

    public func update(from remote: SearchSavedItemParts, with space: Space) {
        remoteID = remote.remoteID
        url = URL(string: remote.url)
        createdAt = Date(timeIntervalSince1970: TimeInterval(remote._createdAt))
        deletedAt = remote._deletedAt.flatMap(TimeInterval.init).flatMap(Date.init(timeIntervalSince1970:))
